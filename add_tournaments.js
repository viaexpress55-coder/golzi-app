...
// ── 1. TournamentScreen.tsx ──────────────────────────────────
writeFile('src/screens/league/TournamentScreen.tsx', `import React, { useState, useEffect } from 'react';
import {
...
// ── 2. Patch LigaScreen ──────────────────────────────────────
const ligaPath = 'src/screens/league/LigaScreen.tsx';
let liga = fs.readFileSync(ligaPath, 'utf8');
...
// Actualizar reglas Firestore para tournaments
const rulesNote = `\n// NOTA: Agregar en Firestore rules:\n// match /tournaments/{tournamentId} { allow read: if request.auth != null; allow write: if request.auth != null && get(/databases/$(database)/documents/leagues/$(leagueId)).data.ownerId == request.auth.uid; }`;

fs.writeFileSync(ligaPath, liga, 'utf8');
console.log('✅ LigaScreen parcheado con tab TORNEOS');
console.log('\n🚀 Script completado.');
